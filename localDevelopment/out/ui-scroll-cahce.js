    ...
    return {
      ...
      compile: function(elementTemplate, attr, linker) {
        return function($scope, element, $attr, controllers) {
          var adapter, adjustBuffer, adjustRowHeight, bof, borderFirst, borderLast, bottomVisiblePos, buffer, bufferPadding, bufferSize, cache, clipBottom, clipTop, datasource, datasourceName, doAdjustment, enqueueFetch, eof, eventListener, fetch, finalize, first, getData, hideElementBeforeAppend, insert, isCacheEnabled, isDatasource, isLoading, itemName, loading, log, match, next, pending, reload, removeFromBuffer, resizeHandler, ridActual, scrollHandler, scrollHeight, shouldLoadBottom, shouldLoadTop, showElementAfterRender, tempScope, topVisible, topVisibleElement, topVisibleItem, topVisiblePos, topVisibleScope, viewport, viewportScope, wheelHandler;
          ...
          isCacheEnabled = $attr.cacheEnable && $attr.cacheEnable !== 'false' ? true : false;
          ...
          borderFirst = null;
          borderLast = null;
          getData = function(from, count, resultCallback) {
            var i, isCached, itemsForCallback, _i, _ref;
            if (!isCacheEnabled) {
              return datasource.get(from, count, resultCallback);
            }
            isCached = true;
            itemsForCallback = [];
            for (i = _i = from, _ref = from + count; _i < _ref; i = _i += 1) {
              if (!cache.hasOwnProperty(i)) {
                if ((borderLast !== null && i > borderLast) || (borderFirst !== null && i < borderFirst)) {
                  continue;
                }
                isCached = false;
                break;
            ...
            if (isCached) {
              return resultCallback(itemsForCallback, true);
            } else {
              return datasource.get(from, count, resultCallback);
            }
          };
